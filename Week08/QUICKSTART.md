# Week 08: Quickstart Guide

This guide describes how to run and test the interactive notebooks and Python scripts across all topics in Week 08.
...
Navigate to any topic folder, for example `01.Introduction-to-Data-Cleaning`:
```bash
cd 01.Introduction-to-Data-Cleaning
```
...
```
Once opened, click on the notebook file (e.g., `01-introduction-to-data-cleaning.ipynb`) and execute the cells sequentially using `Shift + Enter`.

### Option B: Standalone Python Script
To run the equivalent command-line Python script from the topic folder:
```bash
uv run src/IntroductiontoDataCleaning.py
```
*(Replace `IntroductiontoDataCleaning.py` with the corresponding script name for other topics, e.g., `src/HandlingMissingData.py`)*
